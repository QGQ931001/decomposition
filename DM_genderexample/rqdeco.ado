*! version 2.0.0 4july2008 Blaise Melly

program rqdeco, rclass byable(recall)
	version 9.2
*1 replay
	if replay(){
		if "`r(cmd)'" != "rqdeco" {
...
*2.3 call the estimation command
		...
		mat colnames `results'=quantile fitted_o counterfactual fitted_r total_differential  characteristics coefficients
		local nq=rowsof(`quants')
...
*2.4.b: bootstrap
	...
	*loop for each replication
			while `j'<=`reps'{
				...
				if (`rc'==0) {
					quietly drop _all
					mat `res'=r(results)
					mat `res1'=vec(`res'[1...,5])'
					quietly svmat `res1'
					mat `res2'=vec(`res'[1...,6])'
					quietly svmat `res2'
					mat `res3'=vec(`res'[1...,7])'
					quietly svmat `res3'
					if( `j'==1){
	...
	* evaluate the boostrap results		
			...
			mat colnames `se'=total_differential characteristics coefficients
			use "`data'"
		}
	}
...
*3.2 Results for each quantile 
		while `k'<=`nq'{
			...
			dis as text _column(0) %17s "Raw difference" as text _column(18) "{c |}" _column(21) as result %8.0g (`results'[`k',5]) _column (31) as result %8.0g (`se'[`k',1]) _column (42) as result %3.2f (`results'[`k',5]/`se'[`k',1]) _column(51) as result %4.3f chi2tail(1,(`results'[`k',5]/`se'[`k',1])^2) _column(59) as result %8.0g (`results'[`k',5]-invnormal(0.5*(1+`level'/100))*`se'[`k',1]) _column(70) as result %8.0g (`results'[`k',5]+invnormal(0.5*(1+`level'/100))*`se'[`k',1])
			dis as text _column(0) %17s "Characteristics" as text _column(18) "{c |}" _column(21) as result %8.0g (`results'[`k',6]) _column (31) as result %8.0g (`se'[`k',2]) _column (42) as result %3.2f (`results'[`k',6]/`se'[`k',2]) _column(51) as result %4.3f chi2tail(1,(`results'[`k',6]/`se'[`k',3])^2) _column(59) as result %8.0g (`results'[`k',6]-invnormal(0.5*(1+`level'/100))*`se'[`k',2]) _column(70) as result %8.0g (`results'[`k',6]+invnormal(0.5*(1+`level'/100))*`se'[`k',2])
			dis as text _column(0) %17s "Coefficients" as text _column(18) "{c |}" _column(21) as result %8.0g (`results'[`k',7]) _column (31) as result %8.0g (`se'[`k',3]) _column (42) as result %3.2f (`results'[`k',7]/`se'[`k',3]) _column(51) as result %4.3f chi2tail(1,(`results'[`k',7]/`se'[`k',3])^2)  _column(59) as result %8.0g (`results'[`k',7]-invnormal(0.5*(1+`level'/100))*`se'[`k',3]) _column(70) as result %8.0g (`results'[`k',7]+invnormal(0.5*(1+`level'/100))*`se'[`k',3])
			if `k'<`nq' {
				dis as text "{hline 17}" "{c +}" "{hline 59}" 
			} 
			else {
...
//calculate the quantiles of interest
	RQ_deco_ReT=RQ_deco_QaN,bm_quantile(vec(select(RQ_deco_Po,RQ_deco_By)),vec(RQ_deco_WeO),RQ_deco_QaN),bm_quantile(vec(select(RQ_deco_Pr,RQ_deco_By)),vec(RQ_deco_WeO),RQ_deco_QaN),bm_quantile(vec(select(RQ_deco_Pr,RQ_deco_By:==0)),vec(RQ_deco_WeR),RQ_deco_QaN)
//calculate the effects of interest
	RQ_deco_ReT=RQ_deco_ReT,RQ_deco_ReT[|(1,2)\(rows(RQ_deco_ReT),2)|]-RQ_deco_ReT[|(1,4)\(rows(RQ_deco_ReT),4)|],RQ_deco_ReT[|(1,3)\(rows(RQ_deco_ReT),3)|]-RQ_deco_ReT[|(1,4)\(rows(RQ_deco_ReT),4)|],RQ_deco_ReT[|(1,2)\(rows(RQ_deco_ReT),2)|]-RQ_deco_ReT[|(1,3)\(rows(RQ_deco_ReT),3)|]
//return the results
...
//calculate the standard errors
	...
	RQ_deco_Se=J(0,3,.)
	nq=rows(RQ_deco_QaN)
	for(i=1;i<=nq;i++) RQ_deco_Se=RQ_deco_Se\(RQ_deco_Se1[i],RQ_deco_Se1[nq+i],RQ_deco_Se1[2*nq+i])
//return the results
